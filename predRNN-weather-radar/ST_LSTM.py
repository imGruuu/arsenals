...
class SpatioTemporal_LSTM(nn.Module):
    ...
    def _compute_cell(self, x, h, c, m):
        ...
        g_ = torch.tanh(self.conv_xg_(x) + self.conv_cg_(c) + self.conv_mg_(m))
        i_ = torch.sigmoid(self.conv_xi_(x) + self.conv_ci(c) + self.conv_mi_(m))
        f_ = torch.sigmoid(self.conv_xf_(x) + self.conv_cf(c) + self.conv_mf_(m))
        m = f_ * torch.tanh(self.conv_mm(m)) + g_ * i_

        o = torch.tanh(self.conv_xo(x) + self.conv_co(c) + self.conv_mo(m))
        h = o * torch.tanh(self.conv_ch(c) + self.conv_mh(m))
...
class HighwayUnit(nn.Module):
    ...
    def forward(self, input_, z=None):
        ...
        p = torch.tanh(self.conv_hp(input_) + self.conv_zp(z))  # information to add
        u = torch.sigmoid(self.conv_hu(input_) + self.conv_zu(z))  # replace gate
        z = u * p + (1 - u) * z
        return z
...
def create_conv(unit, kind):
    if kind == 'hh':
        padding = (int((unit.kernel_HW[0] - 1) / 2),
                   int((unit.kernel_HW[1] - 1) / 2))
        return nn.Conv2d(unit.hidden_size[0], unit.hidden_size[0],
                         unit.kernel_HW, padding=padding)
    elif kind == 'ih':
        padding = (int((unit.hidden_size[1] - unit.input_size[1] + unit.kernel_HW[0] - 1) / 2),
                   int((unit.hidden_size[2] - unit.input_size[2] + unit.kernel_HW[1] - 1) / 2))
        return nn.Conv2d(unit.input_size[0], unit.hidden_size[0],
                         unit.kernel_HW, padding=padding)
